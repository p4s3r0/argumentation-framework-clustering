...
The argument \texttt{<path\_af>} is the path to the AF, of which the semantics extensions should be computed. To set the semantics of the extension check the additional flags paragraph below.
...
\paragraph{Additional Flags} To have full control of the inner workings of the program and the output printed to the user, we implemented additional flags. The first flag is the semantics flag.
...
The listing of the arguments which need to be concrete is specified after the \texttt{-p} tag and is specified in a joint list separated by spaces.
...
If the refinement of the current semantics is not desired, it can be disabled with the following flag.

\begin{center}
    \texttt{-noref}
\end{center}

We also implemented a visualization of the AFs, which can be triggered by using the the sollowing flag.

\begin{center}
    \texttt{-vis}
\end{center}

The visualisation will show the abstract AF, the concrete AF if it is part of the program call and finally it will show the resulting AF. To ''spectate`` the current computation and have more insight to the algorithm, we can add the \texttt{-verbose} flag. This will print interim findings to the user directly while calculating.

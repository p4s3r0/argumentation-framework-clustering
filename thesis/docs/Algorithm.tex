...
The three components (i.d.\ $\overline{cf}$, $\overline{att}$, $\overline{con}$) represent three different conditions which together define the refinement. Here, $\overline{cf}$ is the negated part of conflict-freeness of the concrete AF, $\overline{att}$ describes, that we want to ignore the concrete stable sets which impliy that if and argument being outside the stable set, has an attacker inside the stable set. Finally, $\overline{con}$ is the negated form of if the stable extension is not attacking an argument, then every singleton atacking the argument is not in the abstractly stable extension.

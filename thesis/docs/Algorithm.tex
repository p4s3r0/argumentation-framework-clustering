...
By comparing the models of $\hat{stb}(\hat{G})$ with the stable sets of $\hat{G}$, we find that they are identical. Specifically, \cref{def:booleanFormulaStable} is a Boolean expression to derive admissible sets from an AF. If the AF has no clusters, the Boolean expression reduces to the admissible formula defined earlier.
...
The issue with the list comparison of the computed extension is, that for AFs with big clusters (i.e.\ a cluster containing many arguments), the direct mapping of a an abstract extension is tedious. This is accounted to the many possibilities a cluster can transform into, e.g., for a cluster $\hat{h}$ containing the arguments $a$ and $b$, the extension $\{\hat{h}\}$ would have to be transformed into $\{a\}$, $\{b\}$ and $\{a, b\}$. This works fine for small clusters, but grows exponentially by $2^{cn}-1$ with $cn$ being the amount of clustered argument.
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \draw[line width=0.3mm] (\ax, \ay) circle (0.3) node[anchor=center]{$a$};
        \draw[cYellow, densely dotted, line width=0.3mm] (\ax,\ay) circle (0.4);
        ...
        \draw[line width=0.3mm] (\cx, \cy) circle (0.3) node[anchor=center]{$c$};
        \draw[cYellow, densely dotted, line width=0.3mm] (\cx,\cy) circle (0.4);
        \node[text=cYellow] at (\cx+0.55, \cy) {\footnotesize 1};

        \draw[line width=0.3mm] (\dx, \dy) circle (0.3) node[anchor=center]{$d$};
        \draw[cBlue, densely dashed] (\dx,\dy) circle (0.4);
        \node[text=cBlue] at (\dx+0.33, \dy+0.45) {\footnotesize 1};

        \draw[line width=0.3mm] (\ex, \ey) circle (0.3) node[anchor=center]{$e$};
        \draw[cBlue, densely dashed] (\ex,\ey) circle (0.4);
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State $N$ $\gets$ $list()$ \Comment{$N$ = list of neighbours}
        \State $n(1) \gets list()$ \Comment{List of neighbours with depth 1}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
        ...
        \node[rectangle, draw, line width=0.3mm] at (\gx, \gy) {$a, b, c, d, e, f$};
        \node at (\gx + 0.1, \gy+0.55) {$\hat{g}$};
        % Attacks
        \DrawSelfAttackLeftTopCluster{\gx-1}{\gy+0.3}
...
When computing the stable extensions of $G$, we obtain $\bigl\{b, c, d\bigl\}$, for the abstract AF $\hat{G}$ we get the abstractly stable extension $\{\hat{g}\}$. Since at least one of the abstractly stable extensions are spurious, we have a spurious abstract AF. Now, we can have a look at the subsets of the spurious extensions and check if every subset is spurious as well. When trying to map the subset of $\{\hat{g}\}$, e.g.\ $\{b, c, d\}$ to the concrete extension $\{b, c, d\}$, we can see that it is faithful. Thus, we found a subset of a spurious extension, which is not spurious.
\end{example}
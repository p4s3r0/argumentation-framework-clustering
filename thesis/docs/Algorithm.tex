...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \For{$e_i$ in $e$}
            \For{$att_i$ in $C[e_i].attacks$} \Comment{Add attacks (argument, cluster)}
                \If{$att_i$ to cluster $c_i$}
                    \State $N.addAttack((e_i, c_i))$
                \EndIf
            \EndFor
            \For{$def_i$ in $C[e_i].defends$} \Comment{Add attacks (cluster, argument)}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \Require $N: AF(a_3, r_3)$ \Comment{Concretized Cluster}
        \For{$r_i$ in $A_{r_1}$}
            \If{$r_i.defender$ is cluster and $r_i.attacker$ is cluster} \Comment{Part 1}
                \For{$a_i$ in $A[r_i.attacker]$}
                    \If{!($a_i$ attacks any of $C[r_i].defender$)}
                        \State $N.removeAttack(r_i)$
                        \State break
                    \EndIf
                \EndFor
            \EndIf
            \If{$r_i.defender$ is cluster} \Comment{Part 2}
                \If{!($r_i.attacker$ attacks any of $C[r_i].defender$)}
                    \State $N.removeAttack(r_i)$
                    \State continue
                \EndIf
            \EndIf
            \If{$r_i.attacker$ is cluster} \Comment{Part 3}
...
\begin{example}
    Let us define $G=(A,R)$ to be a concrete AF, with the arguments $A=\{a, b, c, d, e, f\}$ and the attacks $R=\bigl\{ (b, a)$, $(d, a)$, $(a, d)$, $(a, c)$, $(c, e)$, $(c, f)$, $(d, f) \bigl\}$ depicted in \cref{fig:AlgorithmRefutedTheoryDefinition}(a). The spurious abstract AF $\hat{G}=(\hat{A}, \hat{R})$ is defined with the arguments $\hat{A}=\{\hat{g}, f\}$, with the cluster $\hat{g}$ containing the arguments $\{a, b, c, d, e\}$ and the attacks being $\hat{R}=\bigl\{(\hat{g}, \hat{g})$, $(\hat{g}, f)\bigl\}$ depicted in \cref{fig:AlgorithmRefutedTheoryDefinition}(b).
